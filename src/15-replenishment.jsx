...
function readyReplenishmentDoc(readyByPlant, funds, plantTitle, generatedBy) {
  const col = (key, label, opt = {}) => ({ key, label, align: opt.align || (opt.money ? "right" : "left"), money: !!opt.money, width: opt.width });
  const columns = [
    col("kind", "Type"), col("ref", "Voucher / Reimb No."), col("employee", "Employee"), col("branch", "Branch"),
    col("checked", "Custodian Approved", { width: "20%" }), col("final", "Final Approval", { width: "20%" }),
  ...
  readyByPlant.forEach(([plantCode, items]) => {
    ...
    items.forEach((x) => {
      count++;
      rows.push({ kind: readyKindLabel(x), ref: x.ref, employee: x.employee, branch: plantLabel(x.branchCode) || x.branchCode,
      ...
      <div className="pcp-content">
        ...
        <div className="pcp-card pcp-card-pad" style={{ marginBottom: 14 }}>
          <div className="pcp-section-title" style={{ margin: "0 0 8px" }}>
            ...
            {/* Export what is on screen — follows the selected plant tab. */}
            <span style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
              <button className="pcp-btn pcp-btn-sm" disabled={!ready.length} title="Print or save as PDF"
                onClick={() => printReportDocument(readyReplenishmentDoc(readyByPlant, funds, plantTitle, generatedBy))}>
                <Printer size={12} /> Print / PDF
              </button>
              <button className="pcp-btn pcp-btn-sm" disabled={!ready.length} title="Export to Excel"
                onClick={() => exportReportExcel(readyReplenishmentDoc(readyByPlant, funds, plantTitle, generatedBy))}>
